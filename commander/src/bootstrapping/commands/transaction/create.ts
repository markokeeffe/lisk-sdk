...
const getParamsObject = async (
	...
	const rawParams = flags.params ? JSON.parse(flags.params) : await getParamsFromPrompt(paramsSchema);
...
const validateAndSignTransaction = (
	...
	const paramsSchema = getParamsSchema(schema, transaction.moduleID, transaction.commandID) as Schema;
...
const createTransactionOnline = async (
	...
	const account = await client.invoke<{ nonce: string }>('auth_getAuthAccount', { address: address.toString('hex') })
	const params = await getParamsObject(registeredSchema, flags, args);
	...
	if (flags.nonce && BigInt(account.nonce) > BigInt(flags.nonce)) {
		throw new Error(
			`Invalid nonce specified, actual: ${
				flags.nonce
			}, expected: ${account.nonce.toString()}`,
		);
	}
